param(
    [string]$InputPath = 'test-results/samples/golden-synthetic.pptx',
    [string]$OutputDirectory = 'test-results/powerpoint-render',
    [string]$ReportPath = 'test-results/powerpoint-smoke.json'
)
...
$renderPath = Join-Path $taskRoot $OutputDirectory
...
try {
    ...
    @{status='PASS'; application='Microsoft PowerPoint'; slides=$checkedDeck.Slides.Count; opened_read_only=$true; export='PNG'; sha256=(Get-FileHash -LiteralPath $samplePath -Algorithm SHA256).Hash.ToLower()} | ConvertTo-Json | Set-Content -Encoding UTF8 $ReportPath
} finally {

...
def build():
    ...
    out = ROOT / "release/release-bundle" / ("capstone-0.1.0-" + stamp)
    ...
    if state["status"] == "PASS":
        ...
        for tag in tags:
            inspection = subprocess.run([docker, "image", "inspect", tag], capture_output=True, text=True)
        ...
        missing += ["Docker image tar + built image digest (host virtualization unavailable)"]
    missing += [
        "ClamAV approved signature database + real scan integration result",
        "Clean-host no-egress installation and rollback evidence",
        "Container OS package SBOM",
    ]
    packages = []
    ...
    sbom = {
        ...
        "creationInfo": {
            ...
            "creators": ["Tool: capstone-nps-bundle-0.1.0"],
        },
        "packages": packages,
        "relationships": [
            {
        ...
        "documentComment": "Python and npm dependency inventory. Container OS inventory unavailable until images can be built.",
    }
...
if __name__ == "__main__":
    build()
    # Never return a successful release exit code for incomplete materials.
    raise SystemExit(2)
